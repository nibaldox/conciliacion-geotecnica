...
if st.session_state.step >= 4 and st.session_state.comparison_results:
    ...
    with tab_table:
        ...
        rows = []
        for c in st.session_state.comparison_results:
            rows.append({
                "Sección": c['section'],
                "Sector": c['sector'],
                "Banco": c['bench_num'],
                "H. Dise (m)": c['height_design'],
                "H. Real (m)": c['height_real'],
                "ΔH (m)": c['height_dev'],
                "Est. Altura": c['height_status'],
                "Ang. Dise (°)": c['angle_design'],
                "Ang. Real (°)": c['angle_real'],
                "ΔAng (°)": c['angle_dev'],
                "Est. Ángulo": c['angle_status'],
                "Berma Real (m)": c['berm_real'],
                "Est. Berma": c['berm_status'],
            })
        import pandas as pd
    ...
    with tab_dash:
        ...
        if res:
            ...
            with c1:
                st.subheader("Cumplimiento por Parámetro")
                # Prepare data for bar chart
                counts = {
                    "Altura": df['height_status'].value_counts(),
                    "Ángulo": df['angle_status'].value_counts(),
                    "Berma": df['berm_status'].value_counts(),
                }
                
                # Combine into a single DF for Plotly
                data_list = []
                for param, series in counts.items():
                    for status, count in series.items():
                        data_list.append({'Parámetro': param, 'Estado': status, 'Cantidad': count})
                ...
                color_map = {
                    "CUMPLE": "green",
                    "FUERA DE TOLERANCIA": "orange",
                    "NO CUMPLE": "red"
                }
                
                fig_bar = px.bar(df_counts, x="Parámetro", y="Cantidad", color="Estado",
            ...
            with c2:
                ...
                # Histograms
                tab_h, tab_a = st.tabs(["Error Altura", "Error Ángulo"])
                with tab_h:
                    fig_hist1 = px.histogram(df, x="height_dev", nbins=20, 
                                            title="Distribución Error Altura (m)",
                                            color_discrete_sequence=['royalblue'])
                    st.plotly_chart(fig_hist1, use_container_width=True)
                with tab_a:
                    fig_hist2 = px.histogram(df, x="angle_dev", nbins=20, 
                                            title="Distribución Error Ángulo (°)",
    ...
    with tab_export:
        ...
        if st.button("📊 Generar Informe Excel"):
            fname = f"Conciliacion_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx"
            # Using a temporary file to save excel then read back
            tmp_excel = f"/tmp/{fname}"
            export_results(
                st.session_state.comparison_results,
                st.session_state.params_design,
                st.session_state.params_topo,
                tolerances,
                tmp_excel, project_info
            )
